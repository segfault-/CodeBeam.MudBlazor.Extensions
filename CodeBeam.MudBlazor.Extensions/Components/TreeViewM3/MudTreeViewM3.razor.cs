...
{
    public partial class MudTreeViewM3<T> : MudComponentBase
    {
        private MudTreeViewItemM3<T> _selectedValue;
        private ICollection<MudTreeViewItemM3<T>> _selectedValues;
        private List<MudTreeViewItemM3<T>> _childItems = new();

        protected string Classname =>
        new CssBuilder("mud-treeview-m3")
          .AddClass("mud-treeview-m3-dense", Dense)
          .AddClass("mud-treeview-m3-hover", Hover)
          .AddClass($"mud-treeview-m3-selected-{Color.ToDescriptionString()}")
           .AddClass($"mud-treeview-m3-checked-{CheckBoxColor.ToDescriptionString()}")
          .AddClass(Class)
        .Build();
        ...
        [CascadingParameter] MudTreeViewM3<T> MudTreeRoot { get; set; }

        [Parameter]
        [Category(CategoryTypes.TreeView.Data)]
        public Func<T, Task<ICollection<T>>> ServerData { get; set; }

        public MudTreeViewM3()
        {
            MudTreeRoot = this;
        }
        ...
        internal async Task UpdateSelectedAsync(MudTreeViewItemM3<T> item, bool requestedValue)
        {
            ...
            if (_selectedValue == item && !requestedValue)
            {
                ...
                await item.SelectAsync(requestedValue);
            ...
            {
                await _selectedValue.SelectAsync(false);
            }
        ...
        internal Task UpdateSelectedItemsAsync()
        {
            _selectedValues ??= new List<MudTreeViewItemM3<T>>();
        ...
        public async Task SelectAsync(MudTreeViewItemM3<T> item, bool isSelected = true)
        {
            await UpdateSelectedAsync(item, isSelected);
        }

        internal void AddChild(MudTreeViewItemM3<T> item) => _childItems.Add(item);
